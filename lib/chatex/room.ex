defmodule Chatex.Room do
  ...
  def leave(room_name, username) do
    session_pid = Chatex.Session.pid(username)
    GenServer.call(via_global(room_name), {:leave, session_pid})
  end

  def send_message(room_name, username, message) do
    GenServer.cast(via_global(room_name), {:send_message, username, message})
  end

  def get_messages(room_name) do
    GenServer.call(via_global(room_name), :get_messages)
  end

  defp via_global(room_name), do: {:global, {:room, room_name}}
  ...
  def handle_cast({:send_message, username, message}, state) do
    ...
    Enum.each(state.sessions, fn session ->
      send(session, {:new_message, Map.put(msg, :room_name, state.name)})
    end)
